   ...
   ##not used
    ...
    def build_g_gpu(self, gpu_idx):
        ...
        with tf.device('/gpu:%d' % gpu_idx), tf.name_scope('gpu_%d' % gpu_idx) as scope:
            ...
            self.net, weights = AGE_MODEL.get_model(point_cloud_slice, self.is_train_pl, bn_decay=self.bn_decay,feature=feature_slice)
            self.loss = AGE_MODEL.get_loss(self.net,label_slice,weights)
            ##gradients
