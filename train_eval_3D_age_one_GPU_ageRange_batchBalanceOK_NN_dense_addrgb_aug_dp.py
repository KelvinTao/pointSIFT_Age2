...
import models.pointSIFT_pointnet_age_dense_rgb as AGE_MODEL
...
class ClassTrainer(object):
    ...
    def get_group10(self,c_age):
        labeln=np.sum(c_age[:,:,0],axis=1).astype(int)+52
    ...
    def load_data_npz(self,npz_path):
        d = np.load(npz_path)
        return [d["points_set"],d["rgb_set"],d["age_label_set"]]
        ##return [d["points_set"][0:3,...],d["age_label_set"][0:3,...]]
 
    def clean_data(self,train_data):
        ##
        points, rgb, c_age =train_data
        ages=np.sum(c_age[:,:,0],axis=1).astype(int)
        indexes=np.where((ages <= self.max_age) & (ages >= self.min_age))[0]
        return [points[indexes,...],rgb[indexes,...],c_age[indexes,...]]
    ...
    def load_data(self):
        ...
        #train_data=self.load_data_npz(DATA_PATH+'/RS.train.4108.8192.npz')#[["points"], ['feature'],["age_label"]]
        ...
        self.train_data=train_data #['image','feature','age_label','sample_weights','groupIndexes'] 
        ##test sample weights
    ...
    def get_batch(self, dataset, idxs, start_idx, end_idx):
        ...
        batch_label = np.zeros((bsize, NUM_CLASS//2,2), dtype=np.int32)
        batch_smpw = np.zeros((bsize), dtype=np.float32)
        ps0, fea0, seg0, smpw0 = dataset  #[list]##for test
        for i in range(bsize):
            idxsUse=idxs[i + start_idx]
    ...
    def build_g_cpu(self):
        ...
        self.point_pl, self.feature_pl, self.label_pl, self.smpws_pl = AGE_MODEL.placeholder_inputs(self.batch_sz, self.point_sz,NUM_CLASS)
        ...
        AGE_MODEL.get_model(self.point_pl, self.is_train_pl, num_class=NUM_CLASS, bn_decay=self.bn_decay,feature=self.feature_pl)
    ...
    def build_g_gpu(self, gpu_idx):
        ...
        with tf.device('/gpu:%d' % gpu_idx), tf.name_scope('gpu_%d' % gpu_idx) as scope:
            ...
            self.net, end_point = AGE_MODEL.get_model(point_cloud_slice, self.is_train_pl, num_class=NUM_CLASS, bn_decay=self.bn_decay,feature=feature_slice)
