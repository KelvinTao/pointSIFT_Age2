...
def get_model(point_cloud, is_training, bn_decay=None, feature=None):
    ...
    dense = tf_util.dropout(dense, keep_prob=0.5, is_training=is_training, scope='dp')
    #pred,weights = tf_util.fully_connected2(dense,1,scope='layer8_dense',activation_fn=None,bn=False,bn_decay=bn_decay,is_training=is_training)#logits
    pred,weights = tf_util.fully_connected2(dense,1,scope='layer8_dense',activation_fn=None,bn=True,bn_decay=bn_decay,is_training=is_training)#logits
    return pred,weights
...
def get_loss(pred,labels,weights):
    """
    :param pred: B
    :param labels: B
    :param smpw: B ; sample weight
    ...
    eps = 0.5
    lam = 0.05
    hinge_loss = tf.reduce_mean(tf.maximum(tf.abs(pred - labels) - eps, 0))
    l2_loss = lam * tf.reduce_sum(weights ** 2)
