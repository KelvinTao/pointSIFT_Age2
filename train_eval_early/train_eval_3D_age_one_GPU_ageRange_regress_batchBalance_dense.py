...
class ClassTrainer(object):
    ...
    def calweights(self,c_age):
        ...
        for i in range(len(mydict)):
            if lenth[i]>0:
                ...
                #sample_weights[mydict[i]]=max(min(round(meanle/lenth[i],2),10),0.2)
    ...
    def load_data(self):
        ##load train and test; 
        ##list:["image"], ["age_label"]
        ##image: Nsample x Npoints x Nchannel; channel: X Y Z (+ R G B)
        ##label: Nsample
        train_data=self.load_data_npz(DATA_PATH+'/sampleTrain_Age_21000.npz')#[["image"], ["age_label"]]
        ...
        ##
        test_data=self.load_data_npz(DATA_PATH+'/sampleTest1000page_Age_21000.npz')##list[[np],[np]]
    ...
    def get_learning_rate(self):
        learning_rate = tf.train.exponential_decay(LEARNING_RATE,
                                                   ...
                                                   DECAY_RATE,
                                                   staircase=True)
        learning_rate = tf.maximum(learning_rate, 1e-3)
    ...
    def build_g_cpu(self):
        ...
        self.point_pl, self.label_pl, self.smpws_pl = AGE_MODEL.placeholder_inputs(self.batch_sz, self.point_sz)
        ...
        AGE_MODEL.get_model(self.point_pl, self.is_train_pl, bn_decay=self.bn_decay)
    ...
    def build_g_gpu(self, gpu_idx):
        ...
        with tf.device('/gpu:%d' % gpu_idx), tf.name_scope('gpu_%d' % gpu_idx) as scope:
            ...
            self.net, end_point = AGE_MODEL.get_model(point_cloud_slice, self.is_train_pl, bn_decay=self.bn_decay)
    ...
    def build_graph(self):
        with tf.device('/cpu:0'):
            ...
            tf.summary.scalar('MAE_weighted', self.MAE_wt)
