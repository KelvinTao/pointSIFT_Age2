...
class ClassTrainer(object):
    ...
    def clean_data(self,train_data,min_age,max_age):
        ##
        points, c_age =train_data
        ages=np.sum(c_age[:,:,0],axis=1).astype(int)
        indexes=np.where((ages <= max_age) & (ages >= min_age))[0]
    ...
    def load_data(self):
        ...
        train_data=self.clean_data(train_data,self.min_age,self.max_age)##50-90:4054 used
    ...
    def get_batch_balance(self, dataset, idxs, start_idx, end_idx):
        a=0
