...
class ClassTrainer(object):
    ...
    def calweights(self,c_age):
        ...
        mydict=list(map(lambda x:[i for i,age in enumerate(labeln) if age==x],labes))
    ...
    def clean_data(self,train_data):
        ##
        points, c_age =train_data
        ages=np.sum(c_age[:,:,0],axis=1).astype(int)
        indexes=np.where((ages <= self.max_age) & (ages >= self.min_age))[0]
        return [points[indexes,...],c_age[indexes,...]]

    def shuffle_data2(self,data,labels):
    ...
    def load_data(self):
        ...
        train_data=self.load_data_npz(DATA_PATH+'/sampleTrain_NN_21000.npz')#[["image"], ["age_label"]]
        ##keep samples less than
        train_data=self.clean_data(train_data)##50-90:4054 used
        train_data=self.shuffle_data2(train_data[0], train_data[1])
        ##
        #test_data=self.load_data_npz(DATA_PATH+'/RS.test.1000.npz')##list[[np],[np]]
        test_data=self.load_data_npz(DATA_PATH+'/sampleTest1000page_NN_21000.npz')##list[[np],[np]]
        ...
        self.groupIndexes=self.get_group10(train_data[1])
    ...
    def get_batch(self, dataset, idxs, start_idx, end_idx):
        ...
        #use balance sample weights, when distribution of test just same as train set
        batch_smpw=self.group_smpws
        return batch_data, batch_label, batch_smpw
            ...
            with tf.Session(config=config) as sess:
                ...
                for epoch in range(epoch_sz):
                    ...
                    for _iter in range(iter_in_epoch):
                        ...
                        batch_data, batch_label, batch_smpw = self.get_batch_balance(self.train_data) 
