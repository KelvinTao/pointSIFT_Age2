...
import models.pointSIFT_pointnet_age_dense_regress_rgb as AGE_MODEL
...
class ClassTrainer(object):
    def __init__(self):
        ...
        #list for group indexes
        self.groupIndexes=None ##10 groups
        #self.group_smpws=[1,1,1,0.8,0.8,0.8,1,1,1,1] 
        self.group_smpws=np.array([0.8,0.8,0.8,1.0,1.0,1.0,0.8,0.8,0.8,0.8])
    ...
    def get_group10(self,num_age):
        labeln=np.round(num_age)
        #labeln=np.array([98,34,56,76,45,34,23,65])
        threshes=[50,55,59,63,66,69,72,76,80,84,95]##len 11
        groupIndexes= [[] for i in range(10)]
        for i,age in enumerate(labeln):
            for j in range(len(threshes)-1):
                if (age > threshes[j]) and (age <= threshes[j+1]):
                    groupIndexes[j].append(i)
        self.groupIndexes=groupIndexes

   ##not used
    def calweights(self,num_age):
        ##NUM_CLASS=Nage_thresh x 2
        ##input: num_age, Nsample x Nage_thresh x 2 ,catrgorical age
        ##output: 
        ##sample_weights: calculate sample weights in num_age set
        ...
        assert num_age.shape[0]==sum(lenth)
        #assert len(num_age)==sum(lenth)
        meanle=sum(lenth)//len(lenths)
        ##Nsample
        sample_weights=np.ones(len(labeln))
        for i in range(len(mydict)):
            if lenth[i]>0:
                #sample_weights[mydict[i]]=min(round(meanle/lenth[i],2),10)
                sample_weights[mydict[i]]=1
        return sample_weights

    def load_data_npz(self,npz_path):
        d = np.load(npz_path)
        a=d['ages']
        age=(np.array([int(i) for i in a])+52).astype(np.float32) ##
        return [d["points_set"],d["rgb_set"],age]] 
 
    def clean_data(self,train_data):
        points, rgb, num_age =train_data
        indexes=np.where((ages <= self.max_age) & (ages >= self.min_age))[0]
        return [points[indexes,...],rgb[indexes,...],num_age[indexes]]

    def shuffle_data2(self,train_data):
        idx = np.arange(train_data[0].shape[0])
        np.random.shuffle(idx)
        return [train_data[0][idx, ...],train_data[1][idx,...],train_data[2][idx]] 

    def load_data(self):
        ##load train and test; 
        ##list:["image"], d["age_label"]
        ##points: Nsample x Npoints x Nchannel; channel: X Y Z (+ R G B)
        ##feature: Nsample x Npoints x Nchannel; channel: X Y Z (+ R G B)
        ##label: Nsample x N_age_thresh x 2: age_thresh:0,1,2,...,99; [>,<]age_thresh
        train_data=self.load_data_npz(DATA_PATH+'/sampleTrain_NN_idage_xyzrgb_21000.npz')#[["points"]['feature'] ["age_label"]]
        ##keep samples less than
        train_data=self.clean_data(train_data)##50-90:4054 used
        train_data=self.shuffle_data2(train_data)
        test_data=self.load_data_npz(DATA_PATH+'/sampleTest1000page_NN_idage_xyzrgb_21000.npz')##list[[np],[np]]
        ##train sample weights
        costweight=self.calweights(train_data[2])
        train_data.append(costweight)
        self.train_data=train_data #['image','feature','age_label','sample_weights'] 
        ##test sample weights
        maeweight=self.calweights(test_data[2])
        test_data.append(maeweight) ##['image','feature','age_label','mae_weights'] 
        ...
        self.point_sz=self.train_data[0].shape[1]
        self.groupIndexes=self.get_group10(train_data[2])
    ...
    def get_batch_balance_wdp(self, dataset):
        points, feature, labels, _ = dataset
        indexUse=[]
        groupIndexes=self.groupIndexes
        for i in range(len(groupIndexes)):
            idx=random.randint(0,len(groupIndexes[i])-1)
            indexUse.append(groupIndexes[i][idx])
        batch_data=points[indexUse,...]
        batch_feature=feature[indexUse,...]
        batch_label=labels[indexUse]
        batch_smpw=self.group_smpws
        for i in range(points.shape[0]):  
            dropout_ratio = np.random.random() * 0.2#0.875  # 0-0.875
            drop_idx = np.where(np.random.random(self.point_sz) <= dropout_ratio)[0]
            batch_data[i, drop_idx, :] = batch_data[i, 0, :]
            batch_feature[i, drop_idx, :] = batch_feature[i, 0, :]
        return batch_data,batch_feature,batch_label,batch_smpw
        
    def get_batch_balance(self, dataset):
        points, feature, labels, _ = dataset
        indexUse=[]
        groupIndexes=self.groupIndexes
        for i in range(len(groupIndexes)):
    ...
    def build_g_cpu(self):
        ...
        self.point_pl, self.feature_pl, self.label_pl, self.smpws_pl = AGE_MODEL.placeholder_inputs(self.batch_sz, self.point_sz)
        ...
        AGE_MODEL.get_model(self.point_pl, self.is_train_pl, bn_decay=self.bn_decay,feature=self.feature_pl)
    ...
    def build_g_gpu(self, gpu_idx):
        ...
        with tf.device('/gpu:%d' % gpu_idx), tf.name_scope('gpu_%d' % gpu_idx) as scope:
            ...
            feature_slice = tf.slice(self.point_pl, [gpu_idx * BATCH_PER_GPU, 0, 0], [BATCH_PER_GPU, -1, -1])
            label_slice = tf.slice(self.label_pl, [gpu_idx * BATCH_PER_GPU], [BATCH_PER_GPU])
            smpws_slice = tf.slice(self.smpws_pl, [gpu_idx * BATCH_PER_GPU], [BATCH_PER_GPU])
            ##model and loss, net is predicted logits
            self.net, end_point = AGE_MODEL.get_model(point_cloud_slice, self.is_train_pl, bn_decay=self.bn_decay,feature=feature_slice)
            self.loss = AGE_MODEL.get_loss(self.net,label_slice,smpws_slice)
            ##gradients
    ...
    def build_graph(self):
        with tf.device('/cpu:0'):
            ...
            ## predict
            self.predict, self.MAE, self.MAE_wt =AGE_MODEL.eval_pred(self.net,self.label_pl,self.smpws_pl)
            ...
            with tf.Session(config=config) as sess:
                ...
                for epoch in range(epoch_sz):
                    ...
                    for _iter in range(iter_in_epoch):
                        ...
                        batch_data, batch_feature,batch_label, batch_smpw = self.get_batch_balance_wdp(self.train_data)
                        ##aug_data may not work or not good
                    ...
                    if (epoch + 1) % 1 == 0:
                        ...
                        if (epoch + 1) % 5 == 0:
                            np.savetxt(os.path.join(SAVE_PATH, 'age_pred_%d.txt'% (epoch + 1)),predict,delimiter=',',fmt='%.2f')
                            #if (epoch + 1) % 10 == 0:
                            _path = saver.save(sess, os.path.join(SAVE_PATH, "age_model_%d.ckpt" % (epoch + 1)))
                        if mae < best_MAE:
    ...
    def evaluate_one_epoch(self, sess, test_writer, step, epoch):
        ...
        MAE_wt=0       
        ##all test data
        points_test,feature_test, label_test,smpw_test=self.test_data
        ###
        ...
        for _iter in range(iter_num):
            ...
            ##get batch data
            batch_data, batch_feature, batch_label, batch_smpw=self.get_batch(self.test_data, test_idxs, start_idx, end_idx)
